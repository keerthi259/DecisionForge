...
console.log(
  `Policy contract coverage: line ${(lineRate * 100).toFixed(2)}%, branch ${(branchRate * 100).toFixed(2)}%.`,
);

if (lineRate < minimumLineRate || branchRate < minimumBranchRate) {
  console.error(
    "Policy contract coverage gate failed: required line 95.00% and branch 90.00%.",
  );
  process.exit(1);
}

console.log("Policy contract coverage gate passed.");
