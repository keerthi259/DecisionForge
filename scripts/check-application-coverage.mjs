...
console.log(
  `Application coverage: line ${(lineRate * 100).toFixed(2)}%, branch ${(branchRate * 100).toFixed(2)}%.`,
);

if (lineRate < minimumLineRate || branchRate < minimumBranchRate) {
  console.error(
    "Application coverage gate failed: required line 85.00% and branch 80.00%.",
  );
  process.exit(1);
}

console.log("Application coverage gate passed.");
